    ...
    public static class Mantissen
    {
        ...
        public static double RoundToMaxMantissaLength(int maxMantissaLength, double digit)
        {
            if (Active)
            {
                ...
                string absDigitStringWithExponent = absDigit.ToString(NumberFormatInfo.InvariantInfo);
                string[] absDigitWithExponentStringArray = absDigitStringWithExponent.Split('E');
                string absDigitString = absDigitWithExponentStringArray[0];

                if (absDigitString.Length < maxMantissaLength + 2)
                {
                    if (sign == -1)
                    {
                        return double.Parse("-" + absDigitStringWithExponent, NumberFormatInfo.InvariantInfo);
                    }

                    return double.Parse(absDigitStringWithExponent, NumberFormatInfo.InvariantInfo);
                }

                if (-temporaryAbsoluteExponent < 0)
                {
                ...
                double temporaryExponentWithBase = Math.Pow(baseDigit, -temporaryAbsoluteExponent);
                double result;

                if (temporaryExponentWithBase < 1)
                {
                    temporaryExponentWithBase = Math.Pow(baseDigit, temporaryAbsoluteExponent);
